...
默认情况下，`draft` 会复用专用的 GStack X profile：`~/.gstack/x-browser-profile`。这样它可以和其他需要 X 的 Codex skill 共用同一个 X 登录态，而且不会读取您日常 Chrome 的 cookie。

如果出现 X 登录页面，请在打开的浏览器窗口中手动登录，然后回到终端按回车键继续。登录会话会保存在这个专用 profile 中，后续运行无需重复登录。

不要在 GStack X Browser daemon 已经用同一个 profile 打开时运行 `x-article-drafter`。Chromium profile 同一时间只能被一个浏览器进程占用。请先关闭 GStack Browser 窗口，或通过 `--profile <目录>` 使用单独的 profile。
...
| `--profile <目录>` | 自定义浏览器配置目录路径 | `~/.gstack/x-browser-profile` |
| `--browser chrome\|chromium` | 要启动的浏览器类型 | 共享 GStack profile 默认 `chromium`，自定义 profile 默认 `chrome` |
...
通常是因为另一个浏览器进程已经在使用相同的配置目录。若使用共享 GStack X profile，请先关闭 GStack Browser 窗口；也可以通过 `--profile` 参数指定一个独立目录。
